...
export function connect(room: string = "") {
    ...
    socket = io("http://localhost:4000?token=WEB&roomId=" + gameRoomID + "&isPrivate=true")
    ...
    socket.on("drawForFirstTime", (cards: AvailableCard[]) => {
        ...
        setTimeout(() => {
            let tupel = KI_playCard(enemyPlayed, played, availableCards, round, myHealth, enemyHealth);
        ...
        for (let i = 0; i < random; i++) {
            setTimeout(() => {
                ...
                if (i == random - 1) {
                    ...
                    setTimeout(() => {
                        logEnemyPlayedCards();
                        let attacks: [PlayedCard, PlayedCard | null][] | null = KI_attackCard(enemyPlayed, played, myHealth, enemyHealth);
                        if (attacks != null) {
    ...
    socket.on("playerAttacks", (result: Result, myH: number, enemyH: number) => {
        ...
        console.log("Attack Result: " + result + ", my health: " + myHealth + ", enemy health: " + enemyHealth);
    ...
    socket.on("playerAttacksPlayer", (attackingCardKey: string, myH: number) => {
        myHealth = myH;
        console.log("Player attacks player: " + attackingCardKey + ", my health: " + myHealth);
        logEnemyPlayedCards("socket: playerAttacksPlayer");
    });
    socket.on("changeStance", (stance: string, cardKey: string) => {
        console.log("[enemy] stance change: card:" + cardKey + " to stance " + stance);
...
function playCard(card: AvailableCard, stance: Stance = Stance.ATTACK) {
    ...
    let playedCard = {id: card.id, key: card.key, name: card.name, attack: card.attack, defense: card.defense, stance: stance}
...
function attackWith(attackingCard: PlayedCard, defendingCard: PlayedCard | null = null) {
    if (defendingCard != null) {
        socket.emit("playerAttacks", gameRoomID, defendingCard.key, attackingCard.key);
        console.log("Attacking card: " + attackingCard.id + " with card: " + defendingCard.id);
    }else {
...
function logEnemyPlayedCards(method: string = "") {
    ...
    enemyPlayed.forEach((card) => {
        console.log(card.name + " " + card.attack + "/" + card.defense + " stance: " + card.stance);
    });
    console.log("======================================")
}